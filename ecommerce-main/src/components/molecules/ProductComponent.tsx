...
function ProductComponent(props: IProductComponentProps) {
  ...
  const addToCart = ()=>{
    ...
    fetch('http://34.10.50.108:9090/dev/v1/cart/add-to-cart', {
        method: 'POST',
        headers: {
    ...
    .then(response=>{
        if(response.status == 200){
            swal('Attention','Product added to cart');
        }else{
            swal('Error','Product not added to cart');
        }
    }
)
  }
  return (
    ...
    <div className="card product-item border-0 mb-4">
        ...
        <div className="card-footer d-flex justify-content-between bg-light border">
            ...
            <button type='button' onClick={addToCart} className="btn btn-sm text-dark p-0"><i className="fas fa-shopping-cart text-primary mr-1"></i>Add To Cart</button>
        </div>
    </div>
</div>
  )
